...
static void init_gdt(void) {
  ...
  core_tss->ist1 = (uint64_t)_alloc_pages(2, ZONE_ANY);
  ...
  // We aren't switching cs because it is already stored with the value that we
  // need (as guaranteed by limine)
  __asm__ volatile("mov %%ax, %%ds" ::"a"(0x10));
  __asm__ volatile("mov %%ax, %%es" ::"a"(0x10));
  __asm__ volatile("mov %%ax, %%ss" ::"a"(0x10));
}

static void init_tables(void) { init_gdt(); }
