...
uint8_t* move_to_memory_map(uint8_t* multiboot) {
    // Increment the pointer to skip over until ou get to the start of the memory map
    multiboot += 8;
...
memory_map_t get_memory_map(uint8_t* multiboot) {
    ...
    // The plus 16 skips over the sixteen bytes of metadata and goes straight to the entries
